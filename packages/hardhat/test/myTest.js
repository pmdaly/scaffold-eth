...
describe("Staker Contract", function () {
  before(async function () {
    ...
    this.ExternalFactory = await ethers.getContractFactory(
  ...
  it("stake(): can add funds", async function () {
    ...
    await this.staker.stake({
      from: this.alice.address,
      value: toEther("0.5"),
    });
    expect(await this.staker.getStakedAmount(this.alice.address)).to.equal(
      toEther("0.5")
    );
  });
  it("withdraw(): can't withdraw before deadline", async function () {
    ...
    await expect(
      this.staker.withdraw({ from: this.alice.address })
    ).to.be.revertedWith("must wait until the deadline has passed");
  });
